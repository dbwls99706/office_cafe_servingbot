...
class RobotStatusSubscriber(Node):
    """ROS 2 노드: /robot_status를 구독하고 /table_num을 퍼블리시"""
...
class RobotStatusHandler(QObject):
    ...
    def check_status(self):
        """ROS 2 메시지를 주기적으로 확인하여 상태를 업데이트"""
    ...
    def publish_table_num(self, value):
        """PyQt에서 호출할 수 있는 table_num 퍼블리시 메서드"""
...
class EyeWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        eye_width, eye_height = 70, 70  # 눈 크기 (동그랗게 조정)
...
class ServingRobotGUI(QWidget):
    ...
    def __init__(self, ros_handler):
        ...
        # 메시지 또는 퀴즈가 한 번만 표시되도록 상태 플래그 추가
    ...
    def update_status(self, status):
        ...
        if status == "대기중":
            ...
            self.delivery_message_label.setVisible(False)
            self.message_displayed = False
            self.hide_buttons()  # 버튼 숨기기
            return

        if status == "이동 완료":
            # "이동 완료" 상태에서 메시지가 이미 표시되었으면 아무것도 하지 않음
    ...
    def confirm_delivery(self):
        ...
        self.message_displayed = False  # 메시지 플래그 초기화
